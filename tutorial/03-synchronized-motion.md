...
![時間補間で同期：全員が同じ duration で目標到達](../assets/images/ch3_sync_interpolation.png)

距離が短い ch0 は 1 ステップで進む量が小さく、距離が長い ch2 は大きい。
でも **ステップ数と総時間が同じ** なので、3 つとも `duration = 1000 ms` の同じ瞬間にゴールへ着きます。
...
![lerp と smoothstep の比較](../assets/images/ch3_easing.png)

直線補間（青）は等速で動くため、開始と終了が「カクッ」となります。
smoothstep（橙）は両端で速度ゼロになる S 字カーブで、サーボへの負荷が小さく、
電流ピークも下がるので電源にも優しくなります。
